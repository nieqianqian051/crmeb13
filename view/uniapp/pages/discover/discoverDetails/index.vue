<template>
	<view :style="colorStyle">
		<view class="bg--w111-fff">
			<view class="pt-20 pl-20 pr-20">
				<view class="rd-16rpx error pt-26 pb-24 pl-32" v-if="infoData.is_verify == -2">
				...
				<view class="rd-16rpx error pt-26 pb-24 pl-32" v-if="infoData.is_verify == -1">
					<view class="flex-y-center fs-28 fw-500">
						<text class="iconfont icon-a-ic_tanhao1 fs-30"></text>
				...
				<view class="rd-16rpx warning pt-26 pb-24 pl-32" v-if="infoData.is_verify == 0">
					<view class="flex-y-center fs-28 fw-500">
			...
			<view class="p-32 flex-between-center">
				<view class="flex-y-center" @tap="authTo('/pages/discover/discoverUser/index?id=' + infoData.relation_id)">
					<image class="w-60 h-60 rd-50-p111-" :src="infoData.author_image"></image>
					<text class="pl-20 fs-28 fw-500">{{infoData.author}}</text>
				</view>
				<view class="flex-y-center">
					...
					<view v-else>
						<view class="flex-y-center" v-show="!infoData.is_follow" @tap="followChange">
							<view class="w-124 h-52 rd-30rpx theme-border font-num flex-center fs-24">
								<text class="iconfont icon-ic_increase fs-20" ></text>关注
							</view>
						</view>
						<view class="flex-y-center" v-show="infoData.is_follow" @tap="followChange">
							<view class="w-124 h-52 rd-30rpx b-e flex-center fs-24 text--w111-999">已关注</view>
						</view>
					</view>
					<!-- #ifdef H5 || APP-PLUS -->
					<text class="iconfont icon-icon_transmit-2 fs-48 pl-24" @tap="openShare"></text>
					<!-- #endif -->
					<!-- #ifdef MP-WEIXIN -->
					<button class="iconfont icon-icon_transmit-2 fs-48 pl-24" open-type="share" hover-class="none"></button>
					<!-- #endif -->
				</view>
			</view>
			<view class="px-20 content-info">
				<swiper
					class="swiper"
					circular
					...
					<swiper-item v-for="(item,index) in infoData.slider_image" :key="index">
						<view class="swiper-item flex-center">
							<image class="w-full"
								:class="{'rd-24rpx': isAuto}"
								show-menu-by-longpress
								:src="item" :mode="isAuto ?  'aspectFill' : 'heightFix'"
								:style="{height:imageH + 'px'}"
								@tap="proview(item)"></image>
						</view>
					</swiper-item>
				</swiper>
				<view class="fs-32 fw-500 pt-26">{{infoData.title}}</view>
				...
				<view class="pt-36 mb-24 fs-22 text--w111-999">{{infoData.add_time}}</view>
				<view class="w-full pb-40 bb-f5" v-if="infoData.product && infoData.product.length > 1">
					<scroll-view scroll-x="true" class="white-nowrap vertical-middle w-full" show-scrollbar="false">
						<view class="inline-block mr-30" v-for="(item,index) in infoData.product" :key="index">
							<view class="w-414 rd-16rpx p-16 b-e flex-between-center" @tap="authTo('/pages/goods_details/index?id=' + item.id)">
								<image :src="item.image" class="w-104 h-104 rd-8rpx"></image>
								<view class="flex-1 pl-14">
									<view class="w-256 line1 fs-26 pb-20">{{item.store_name}}</view>
									<view class="flex-between-center">
										<baseMoney :money="item.price" symbolSize="22" integerSize="32" decimalSize="24" color="#333" weight></baseMoney>
				...
				<view class="w-full pb-40" v-else-if="infoData.product && infoData.product.length == 1">
					<view class="w-full rd-16rpx p-16 b-e flex-between-center" @tap="authTo('/pages/goods_details/index?id=' + infoData.product[0].id)">
						<image :src="infoData.product[0].image" class="w-104 h-104 rd-8rpx"></image>
						<view class="flex-1 pl-14">
							<view class="w-558 line1 fs-26 pb-20">{{infoData.product[0].store_name}}</view>
							<view class="flex-between-center">
								<baseMoney :money="infoData.product[0].price" symbolSize="22" integerSize="32" decimalSize="24" color="#333" weight></baseMoney>
								<view class="w-96 h-40 rd-20rpx flex-center fs-22 font-num theme-border">购买</view>
							</view>
						</view>
					</view>
			...
			<view class="fixed-lb w-full bg--w111-fff pb-safe z-999" 
				...
				<view class="pl-32 pr-24 h-96 flex-between-center" v-show="!showInput">
					<view class="flex-1 h-64 rd-32rpx bg--w111-f5f5f5 pl-24 flex-y-center text--w111-999 px-20"
						@tap="openInput">
						<text class="fs-26 pl-16 text--w111-999">说说你的看法吧</text>
					</view>
					<view class="flex-y-center pl-30">
						<view class="text-primary-con flex-y-center h-64" @tap="contentLike">
							<text class="iconfont" :class="infoData.is_like ? 'icon-icon_Like_2' : 'icon-ic_Like'"></text>
							<text class="fs-22 pl-8">{{infoData.like_num}}</text>
						</view>
						<view class="pl-32 flex-y-center h-64">
							<text class="iconfont icon-ic_message"></text>
							<text class="fs-22 pl-8">{{infoData.comment_num}}</text>
						</view>
					</view>
				</view>
				<view class="pl-32 pr-24 h-96 flex-between-center" v-show="showInput">
...
// #ifdef APP-PLUS || H5
.content-info /deep/uni-swiper .uni-swiper-dots-horizontal .uni-swiper-dot{
	width: 12rpx;
	height: 12rpx;
	border-radius: 50%;
}
// #endif
// #ifdef MP
.content-info swiper /deep/.wx-swiper-dot{
	width: 12rpx;
	height: 12rpx;
	border-radius: 50%;
}
