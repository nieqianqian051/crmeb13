<template>
	<view :style="colorStyle">
						...
						<view class="mt-20">
							...
							<view class="w-full mt-24 pb-40 bb-f5" v-else-if="item.product && item.product.length == 1">
								<view class="w-full rd-16rpx p-16 b-e flex-between-center" @tap.stop="authTo('/pages/goods_details/index?id=' + item.product[0].id)">
									...
									<view class="flex-1 pl-14">
										<view class="w-558 line1 fs-26 pb-20">{{item.product[0].store_name}}</view>
										<view class="flex-between-center">
		...
		<view class="fixed-lt w-full bg--w111-fff rd-b-24rpx z-100" :style="{top: sysHeight + 38 + 'px'}"
			v-if="showCateDrawer">
			<view class="w-full pt-30 pr-20 pb-30 pl-30">
				<view class="flex-between-center">
		...
		methods: {
			...
			changeTab(val){
				...
				if(val == 0){
					...
					let that = this;
					uni.navigateTo({
						url: `/pages/discover/discoverVideo/app?url=${HTTP_REQUEST_URL}/pages/discover/discoverVideo/index?content_type=0&token=${that.$Cache.get(LOGIN_STATUS)}`
					});
					return
					// #endif
					// #ifdef H5 || MP-WEIXIN
			...
			getTopic(){
				getTopicApi().then(res=>{
					...
					this.topicNoRecmmend = this.topicList.filter(val=> val.is_recommend == 0);
				})
			},
			getList(){
