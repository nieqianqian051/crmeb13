							...
							<view class="w-full pt-36">
								...
								<view class="flex-y-center" v-if="!activityBg">
									<view class="flex items-baseline">
										...
										<view class="fs-22 text--w111-3d3d3d pl-12"> 
											售价<text class="Regular">¥{{ attr.productSelect.price }}</text> 
										</view>
										<view class="svip-badge" v-if="showVipPrice">
											<text class="h-26 lh-28rpx svip_rd fs-18 bg--w111-484643 text--w111-FDDAA4 px-8">SVIP</text>
											<text class="px-8 Regular fw-600">¥{{attr.productSelect.vip_price}}</text>
										</view>
									</view>
								</view>
								<!-- 活动标签 -->
									...
									<scroll-view
										...
										<view
											...
											>
												...
												<text v-show="item.use_min_price != 0">满{{ item.use_min_price }}元可用</text>
											</view>
										<view
											class="inline-block h-44 rd-8rpx text-center lh-44rpx fs-22 text-primary-con px-8 bg-primary-light mr-16"
								...
								<view class="flex-between-center mt-24 text--w111-999 fs-22 lh-30rpx">
									<text v-show="diyProduct.isOpen.includes(0)">划线价：
		...
		methods: {
			...
			ChangeAttr: function(res) {
				...
				if (!productSelect) {
					...
					if (productSelect && productSelect.stock >= 0) {
						this.$set(this.attr.productSelect, 'pay_price', productSelect.delivery_price);
			...
			getGoodsDetails: function() {
				...
				getProductDetail(that.id, {promotions_type: this.promotions_type})
					.then(res => {
						...
						that.$set(that.storeInfo, 'specsTitle', specsTitle.join(' · '));
			...
			DefaultSelect: function() {
				...
				if (productSelect && productAttr.length) {
					...
					this.$set(this.attr.productSelect, 'pay_price', this.storeInfo.price);
					this.$set(this.attr.productSelect, 'image', this.storeInfo.image);
					this.$set(this.attr.productSelect, 'price', this.storeInfo.price);
					this.$set(this.attr.productSelect, 'ot_price', this.storeInfo.ot_price);
					this.$set(this.attr.productSelect, 'stock', 0);
					...
					setTimeout(()=>{
						this.$set(this.attr.productSelect, 'pay_price', this.computedPrice.deduction.pay_price);
					},1000)
					this.$set(this.attr.productSelect, 'image', this.storeInfo.image);
					this.$set(this.attr.productSelect, 'price', this.storeInfo.price);
					this.$set(this.attr.productSelect, 'ot_price', this.storeInfo.ot_price);
			...
			async getImageBase64(type) {
				this.storeImage = await this.downloadFileImage(this.storeImage);
			},
			// 小程序关闭分享弹窗；
			// goFriend: function() {
			// 	this.posters = false;
