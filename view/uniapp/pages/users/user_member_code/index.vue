		...
		<view class="wrapper">
			<view class="wrapper-content">
				<view class="header" :style="[headerStyle]" v-if="currentLevel">
					<view class="title" :style="{ color: currentLevel.color }">尊敬的{{nowLevel.name ? nowLevel.name : ''}}会员</view>
	...
	export default {
		...
		computed: {
			headerStyle() {
				let styleObject = {};
				if (this.nextLevel) {
					styleObject['background'] = `linear-gradient(91deg, ${this.colorToRgba(this.currentLevel.color||'', 0.05)} 0%, ${this.colorToRgba(this.currentLevel.color||'', 0.4)} 100%)`;
				}
				return styleObject;
			}
		},
