	...
	<view :style="colorStyle">
		...
		<view class="px-20" :style="{'margin-top':marTop + 'px'}" v-show="!focus">
			...
			<view class="mt-32 flex-between-center">
				<!-- <text class="fs-24 font-num" v-if="item._status._type == 0">{{item._status._msg}}</text> -->
				<text></text>
				<view class="flex">
					...
					<!-- <view class="btn w-154 h-56 rd-30rpx flex-center fs-24 bg-color text--w111-fff"
						v-if="item._status._type == 4" @tap.stop="showModalChange(item.order_id, 2)">删除订单</view> -->
				</view>
			</view>
		</view>
