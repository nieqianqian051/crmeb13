	...
	<view :style="colorStyle">
		<view class="w-full relative z-99">
			...
			<view class="px-20 relative z-80">
				...
				<view class="bg--w111-fff rd-24rpx p-32 relative z-1" v-if="orderInfo.shipping_type === 2 && [0, 4].includes(orderInfo.product_type)">
					<view class="flex-between-center">
		...
		<view class="fixed-lb bt w-full bg--w111-fff pb-safe z-999">
			<view class="h-96 px-20 flex-y-center justify-end">
				<view
					class="btn w-144 h-56 rd-30rpx flex-center fs-24 border bg--w111-fff"
					v-if="invoice_func && !invoiceData && orderInfo.type != 8 && status.type != -2"
					@tap="invoiceApply"
				>
					申请开票
				</view>
				<view
					class="btn w-144 h-56 rd-30rpx flex-center fs-24 border bg--w111-fff"
					v-if="invoiceData"
					@tap="goPage(1, `/pages/users/user_invoice_order/index?order_id=${orderInfo.order_id}`)"
				>
					查看发票
				</view>
				<view class="btn w-144 h-56 rd-30rpx flex-center fs-24 text--w111-fff bg-color" v-if="status.type == 0" @tap="pay_open()">立即支付</view>
				<view class="btn w-144 h-56 rd-30rpx flex-center fs-24 border bg--w111-fff" v-if="status.type == 0 || status.type == -9" @tap="showModalChange(3)">取消订单</view>
				...
				<view class="btn w-144 h-56 rd-30rpx flex-center fs-24 border bg--w111-fff" v-if="(status.type == 4 && !split.length) || status.type == -2" @tap="showModalChange(2)">
					删除订单
				</view>
				<view class="btn w-144 h-56 rd-30rpx flex-center fs-24 font-num con_border bg--w111-fff" v-if="status.class_status == 5" @tap="goOrderConfirm">再次购买</view>
				<view
