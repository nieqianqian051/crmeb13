...
class CommunityServices extends BaseServices
{
    ...
    public function setCommunityLike(array $info, int $uid, int $status)
    {
        ...
        if ($status) {
            ...
            //帖子点赞数
            $this->dao->incUpdate(['id' => $info['id']], 'like_num');
            //作者获得点赞数
            ...
            $this->decUpdate(['id' => $info['id']], 'like_num');
    ...
    {
        return $this->cacheTag()->remember(CommunityCacheServices::COMMUNITY_TOPIC_COUNT . $id, function () use ($id) {
    ...
    public function syncTopicNum(int $id)
    {
        ...
        foreach ($list as $item) {
            ...
            $this->cacheTag()->set(CommunityCacheServices::COMMUNITY_TOPIC_COUNT . $item['right_id'], $count);
        }
        return true;
    }
}
