    ...
    public function computeOrderProductFirstDiscount(array $cartInfo, array $priceData)
    {
        ...
        if ($first_order_price) {
            ...
            foreach ($cartInfo as &$cart) {
                ...
                if ($count > 1) {
                    $price = bcmul((string)bcmul((string)$cart['cart_num'], (string)$cart['truePrice'], 4), (string)$ratio, 2);
                    $compute_price = bcadd((string)$compute_price, (string)$price, 2);
                } else {
                    $price = bcsub((string)$first_order_price, $compute_price, 2);
                }
                $count--;
                $cart['first_order_price'] = $price;
            }
        }
        return $cartInfo;
    }
