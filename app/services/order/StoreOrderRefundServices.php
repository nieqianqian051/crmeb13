...
//            0 => ['name' => '全部', 'num' => $this->dao->count($del_where)],
     ...
     * @throws \think\db\exception\ModelNotFoundException
     */
    public function applyRefund(int $id, int $uid, $order = [], array $cart_ids = [], int $apply_type = 1, float $apply_price = 0.00, array $refundData = [])
    {
        if (!$order) {
...
//                $res2 = false !== $this->storeOrderServices->update(['id' => $order['id']], ['refund_status' => 1]);
    ...
    public function cancelApplyRefund(int $uid, string $order_id)
    {
        ...
        $this->transaction(function () use ($uid, $orderRefund) {
            ...
            $statusService = app()->make(StoreOrderStatusServices::class);
            $res1 = false !== $statusService->save([
                    'oid' => (int)$orderRefund['store_order_id'],
                    'change_type' => 'cancel_apply_refund',
                    'change_message' => '用户取消申请退款',
                    'change_time' => time()
                ]);
        });
        return true;
