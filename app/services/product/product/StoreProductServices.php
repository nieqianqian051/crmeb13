...
//        }
        ...
        $data['label_id'] = array_column($data['label_id'], "id");
        ...
        if ($productValue) {
            foreach ($productValue as &$item) {
                ...
                if (!$storeInfo['is_presale_product']) {
                    try {
                        $computed = $this->computedProductPayPrice($uid, $id, $item['unique']);
                        $item['delivery_price'] = $computed['deduction']['pay_price'] ?? $item['price'];
                    } catch (\Throwable $e) {
                    }
                }
            }
