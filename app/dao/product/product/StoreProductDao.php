...
class StoreProductDao extends BaseDao
{
    ...
    public function search(array $where = [], bool $search = false)
    {
        return parent::search($where, $search)
                ...
                if (isset($where['field_key']) && in_array($where['field_key'], ['product_id', 'bar_code', 'store_name'])) {
                    if ($where['field_key'] == 'store_name') {
                        $query->whereLike('store_name', '%' . trim($where['store_name']) . '%');
                    } else if ($where['field_key'] == 'product_id') {
